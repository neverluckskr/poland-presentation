...
export default function Footer({ currentSlide, totalSlides }) {
  ...
  return (
    <footer 
      ...
      role="contentinfo"
      aria-label="Информация о прогрессе презентации"
    >
      <div 
        ...
        aria-valuemin="0"
        aria-valuemax="100"
        aria-label={`Прогресс презентации: ${Math.round(progress)}%`}
      >
        <div
      ...
      <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm opacity-85">
        ...
        <span className="font-semibold text-primary" aria-label={`Текущее время: ${time}`}>{time}</span>
      </div>
    </footer>
  );
}
