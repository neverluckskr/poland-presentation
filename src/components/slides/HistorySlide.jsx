  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 gap-9 items-center" style={{ gridTemplateColumns: '1fr 1fr' }}>
        <div className="order-2 grid gap-7">
          {slide.content.periods.map((period, index) => (
            <div key={index}>
              <h3 className="text-3xl text-primary mb-4">{period.title}</h3>
              <ul className="text-xl list-none">
                {period.items.map((item, itemIndex) => (
                  <li key={itemIndex} className="pl-10 py-3 relative transition-all duration-300 hover:translate-x-2 hover:text-primary hover:font-semibold">
